...
- Запуск бота командой:         ./ebotSpot-7g 
...
- в полночь в канал отправляет суточный отчёт о работе. Если бот находится в режиме поиска подходящей монеты, то суточный отчёт в telegram не придёт, а в следующий отчёт будет прибавлена предыдущая прибыль за сутки. Точные данные по прибыли наблюдать лучше в лк binance, так как бот показывает приблизительные значения.
...
- перенесите в эту папку файл бота ebotSpot-7g
- откройте screen-сессию (например:    screen -S ebotSpot   )
- дайте права запуска файла (команда:    chmod 755 ebotSpot-7g   )
- запустите E-Bot (команда:    ./ebotSpot-7g   )
